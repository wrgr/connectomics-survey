...
## 4d. v5 simplification (2026-08-26)

On the screener's direction, the protocol was rewritten as a **lean JBI + PRISMA-ScR scoping review** (`connectomics_bibliography_methodology_v5.md`), exploiting the field's two structural features — few significant datasets (lineage and usage are chartable facts) and a stereotyped pipeline (the charting taxonomy) — in place of v4's importance-claiming machinery, which is withdrawn explicitly (v5 §12): no percentile Core, no probe-panel/convergence protocol objects, no dial-invariance or ablation apparatus, no typed adjudication taxonomy. Development artifacts keep reduced roles: pilot = JBI initial search + comparison set; review pool v2 = vocabulary source; panel + convergence run = one-time supplementary saturation check; COI freeze = disclosure; backbone = historical only. The registration decision in §5 below is unchanged in substance and *smaller in scope*: the deposit now covers a standard protocol, frozen strings, and the dataset registry.
